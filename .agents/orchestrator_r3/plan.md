...
|---|----------------|-------|-------------------------|--------------|--------|
| M1 | Backend Video Engine & Avatar (R1, R4, R5-Backend) | Video speedup (<=20s/min), photorealistic avatar, backend branding | `backend/app/services/video_stitcher.py`, `backend/app/services/avatar_service.py`, `backend/app/services/slide_render_service.py`, `backend/app/config.py`, `backend/app/main.py`, `backend/tests/test_ingestion.py`, `data/avatars/` | None | PLANNED |
| M2 | Frontend Flow & Light Theme (R2, R3, R5-Frontend) | Single 'Generate Video' button, light palette styling, frontend branding | `frontend/src/*`, `frontend/index.html`, `frontend/index.css`, `frontend/tailwind.config.js`, `frontend/package.json` | None | PLANNED |
| M3 | Infrastructure & Documentation (R5-Infra/Docs) | Container names, launcher banners, root README, docs | `docker-compose.yml`, `run.sh`, `README.md`, `docs/*`, `package.json` | None | PLANNED |
| M4 | Comprehensive E2E Verification & Audit Gate | Test suite for R1-R5, run all backend & E2E tests, audit gate | `tests_e2e/*` | M1, M2, M3 | PLANNED |

---

## Interface Contracts & Data Flow
- **Chained Video Generation**:
  - `POST /api/v1/materials/topic` or `POST /api/v1/materials/upload` -> `{ material_id, ... }`
  - `POST /api/v1/lessons/plan` with `material_id` and default learner profile -> `LessonPlan`
  - `POST /api/v1/lessons/generate-video` with `LessonPlan` -> `{ task_id, status: "processing" }`
  - Polling `GET /api/v1/lessons/tasks/{task_id}` -> status `"completed"`, `{ video_id, manifest_url }`
- **Video Generation Performance SLA (R1)**:
  - Total processing time $\le 20\text{s} \times \frac{\text{duration\_sec}}{60.0}$
- **Branding Standard (R5)**:
  - Case-sensitive display: "ApniHelp"
  - Lowercase slug: "apnihelp"
