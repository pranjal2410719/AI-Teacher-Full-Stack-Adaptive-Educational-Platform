## 2026-09-01T10:34:19Z
You are worker_r2_docker_demo_2, an implementation and verification worker replacing worker_r2_docker_demo.
Your working directory is: /home/dev/Desktop/projects/AI-InnovationHackathon/.agents/worker_r2_docker_demo_2
...
3. Update `frontend/Dockerfile` to build Vite app and serve cleanly.
...
5. Enhance `run.sh` so that:
   - It supports starting backend and frontend.
   - It supports `./run.sh --demo` or sample generation mode that executes a full pipeline generating a complete video >= 2 minutes with interactive checkpoints on a sample educational topic (e.g. Calculus Limits or Biology, in English and Hindi), printing the video path and duration.
6. If needed, adjust the microsecond timing SLA in backend benchmark test to prevent transient timing failures.
7. Run tests (`pytest backend/tests/ -v` and `python3 tests_e2e/test_runner.py`) and test running `./run.sh --demo` to verify that a >= 2 min video is generated and all tests pass.
8. Document all changes and verification commands in `/home/dev/Desktop/projects/AI-InnovationHackathon/.agents/worker_r2_docker_demo_2/changes.md` and write `handoff.md`.
